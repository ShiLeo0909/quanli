...
public class MyListVector extends AbstractMyVector {
    ...
    public double[] coordinates() {
        /* TODO */double[] coordinates = new double[size()];
    ...
    public MyListVector add(double value) {
        /* TODO */
        for (int i = 0; i < size(); i++) {
            double newValue = coordinate(i) + value;
            set(newValue, i);
        }
        return this;
    }
    ...
    public MyListVector add(MyListVector another) {
        ...
        for (int i = 0; i < size(); i++) {
            double newValue = coordinate(i) + another.coordinate(i);
            set(newValue, i);
        }
        return this;
    }
    ...
    public double dot(MyListVector  another) {
        ...
        double dotProduct = 0;
        for (int i = 0; i < size(); i++) {
            dotProduct += data.get(i) * another.data.get(i);
        }
    ...
    public MyListVector pow(double power) {
        /* TODO */
        for (int i = 0; i < size(); i++) {
            double newValue = Math.pow(coordinate(i), power);
            set(newValue, i);
        }
        return this;
    }
    ...
    public double norm() {
        ...
        for (int i = 0; i < size(); i++) {
            double coordinate = coordinate(i);
    ...
    public MyListVector extract(int[] indices) {
        /* TODO */
        MyListVector extractedVector = new MyListVector();

        for (int index : indices) {
            if (index < 0 || index >= size()) {
                throw new IllegalArgumentException("Chỉ số không hợp lệ.");
            }
            extractedVector.insert(coordinate(index));
        }
